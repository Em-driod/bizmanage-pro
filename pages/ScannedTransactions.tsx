...
const ScannedTransactions: React.FC = () => {
  ...
  useEffect(() => {
    const handler = () => fetchScannedTransactions();
  ...
  return (
    <div className="min-h-screen space-y-6 sm:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out pb-8">
      ...
      {showDetailsModal && selectedTxForDetails && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-900/50 backdrop-blur-sm overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl animate-in zoom-in duration-200 my-auto">
            <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center">
                ...
                <div>
                  ...
                  <div className="space-y-3 max-h-40 overflow-y-auto">
                    {selectedTxForDetails.parsedDetails.map((item, index) => (
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white border border-slate-200 p-3 rounded-lg shadow-sm">
                        <div className="min-w-0">
                          <p className="text-sm font-medium text-slate-900 truncate">{item.description || 'No Description'}</p>
